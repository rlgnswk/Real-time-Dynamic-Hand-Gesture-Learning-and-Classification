# -*- coding: utf-8 -*-
"""
Created on Wed Feb  5 14:44:55 2020
...
'''
각 버튼에 대한 응답을 조정해줄 요구가 있음 . 지금은 UI 만 합쳐놓은 상태임 0205 
...
class Myapp():
    def __init__(self, window):
        ...
        self.x_points = []
        self.y_points = []
        self.forget_param_x = 0.70
        self.forget_param_y = 0.70
        self.pre_up=0.0
        self.pre_down=0.0 
        self.pre_right=0.0
        self.pre_left=0.0
        ...
        self.window.resizable(False, False)
         ...
         # 현재 상태 글귀 result_txt
        self.State_txt = tkinter.Label(self.window, text="현재 모드(motion)")
        ...
        #제스쳐명 글귀
        self.lbl = tkinter.Label(self.window, text="모션 등록")
        ...
        #start
        self.start = tkinter.Button(self.window, text="start", command=lambda:self.button_pressed_start())
        self.start.place(x=50,y=150)
        #Stop 버튼 생성 클릭 시 listbox에 등록
        self.Stop = tkinter.Button(self.window, text="Stop", command=lambda:self.button_pressed_stop())
        self.Stop.place(x=100,y=150) 
        ...
        ## ==================================================================================================
        # 현재 상태 글귀 result_txt
        self.State_txt_left = tkinter.Label(self.window, text="현재 모드(gesture)")
        self.State_txt_left.place(x=790,y=0) 
        
        
        #현재 상태  출력 창  State
        self.State_left = tkinter.Entry(self.window, justify='center')
        self.State_left.place(x=790, y=20)
        
        # 인식 결과 글귀 result_txt
        self.result_txt_left = tkinter.Label(self.window, text="인식 결과")
        self.result_txt_left.place(x=790,y=40)
        
        #인식결과 출력 창 result_output
        self.result_output_left = tkinter.Entry(self.window, justify='center')
        self.result_output_left.place(x=790, y=60)
        
        self.Test_left = tkinter.Button(self.window, text="     Test     ", command=lambda:self.button_pressed_Test())
        self.Test_left.place(x=790,y=80)
        
        #Stop 버튼 생성 클릭 시 listbox에 등록
        
        self.Stop2_left = tkinter.Button(self.window, text="     Stop     ", command=lambda:self.button_pressed_stop())
        self.Stop2_left.place(x=790+70,y=80) 
        
        #제스쳐명 글귀
        self.lbl_left = tkinter.Label(self.window, text="제스쳐 등록")
        self.lbl_left.place(x=790,y=110)
         
        #제스처 입력창 input_gesture
        self.input_gesture_left = tkinter.Entry(self.window, justify='center')
        self.input_gesture_left.place(x=790,y=130)
        
        #Train 버튼 생성 클릭 시 listbox에 등록
        self.Train_left = tkinter.Button(self.window, text="    Train     ", command=lambda:self.button_pressed_Train())
        self.Train_left.place(x=790,y=150)
        #Stop 버튼 생성 클릭 시 listbox에 등록
        
        self.Stop_left = tkinter.Button(self.window, text="    Stop     ", command=lambda:self.button_pressed_stop())
        self.Stop_left.place(x=790+70,y=150) 
        
        
        # 등록된 제스쳐 글귀
        self.lbl2_left = tkinter.Label(self.window, text="등록된 제스쳐")
        self.lbl2_left.place(x=790,y=180)
        ...
        self.scrollbar["command"]=self.listbox.yview
        self.frame.place(x=790,y=200)
        
        # Reset ,및 delete 버튼 클릭시, listbox 초기화
        # reset button clear the listbox
        self.Reset = tkinter.Button(self.window, text="     Reset    ", command=self.button_pressed_Reset)
                                    
        self.Reset.place(x=790+70,y=380)
        
        # delete button
        self.btn2 = tkinter.Button(self.window, text="    Delete   ", command= self.delete_element)
        self.btn2.place(x=790,y=380)
    ...
    def button_pressed_start(self):
        ...
        if len(input_string) == 0:
            pass
        elif self.pre_string == input_string:
            pass
        else:
...
#            self.listbox.insert(tkinter.END, self.input_gesture.get())
    ...
    #관절 정보를 받아 적절한 인풋으로 변환
        try:
            ...
            # 이전 프레임의 정보와 비교 
            if self.MODE_FLAG ==4 or self.MODE_FLAG == 2 or  self.MODE_FLAG == 1:
                self.x_differ = self.x_differ - x_points[1]
                self.y_differ = self.y_differ - y_points[1]
                print("1.self.pre_right::",self.pre_right)
                if self.x_differ > 0 :
            ...
            # 총 15개의 성분 list + 4개의 방향 
            #return input_data1 + input_data2 + input_data3 + input_data4 + input_data5 + dir_info
