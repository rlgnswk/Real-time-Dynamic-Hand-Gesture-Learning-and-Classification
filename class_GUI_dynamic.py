...
class Myapp():
    def __init__(self, window):
        ...
        self.handData =[]# for artmap input
        
        self.x_points = []# to separate the given info frorm hand recog C++ program
        self.y_points = []# to separate the given info frorm hand recog C++ program
        
        
        ## 손 전체의 움직임을 인식하기 위한 파라미터 
        ...
        # 손 모양의 움직임을 인식하기위한 파라미터 
        self.forget_param_dis = 0.60
        # 0~4 closer info 5 ~ 9 further info 
        self.distance_info = np.zeros(10).tolist()
        self.distance_differ = np.zeros(5).tolist()
        
        self.pre_thumb_further=0.0
        self.pre_index_finger_further=0.0 
        self.pre_middle_finger_further=0.0
        self.pre_ring_finger_further=0.0
        self.pinky_finger_further = 0.0
        
        self.pre_thumb_closer=0.0
        self.pre_index_finger_closer=0.0 
        self.pre_middle_finger_closer=0.0
        self.pre_ring_finger_closer=0.0
        self.pinky_finger_closer = 0.0
        
        #이전의 label 정보 저장을 위한 변수 
...
#            self.listbox.insert(tkinter.END, self.input_gesture.get())
    ...
    #관절 정보를 받아 적절한 인풋으로 변환
        try:
            ...
            #dis_info = input_data1 + input_data2 + input_data3 + input_data4 + input_data5
            
            
            self.distance_differ = [i - j  for i,j in zip(self.distance_differ , input_data1)]#6 엄지 10 검지 14 중지 18약지 22새끼
            ...
            if self.MODE_FLAG ==4 or self.MODE_FLAG == 2 or  self.MODE_FLAG == 1:
                ...
                for idx, i in enumerate(self.distance_differ):
                    if i > 0:
                        self.distance_info[idx] = self.forget_param_dis*self.distance_info[idx] + i
                
                    else:
                        self.distance_info[idx+5] = self.forget_param_dis*self.distance_info[idx+5] - i
            ...
            print("self.distance_info::",self.distance_info[1])
            ...
            self.distance_differ = input_data1
            
            dir_info = [self.pre_up, self.pre_down, self.pre_right, self.pre_left]
            
            #return  dis_info + dir_info# 총 15개의 성분 list + 4개의 방향 변
            return self.distance_info + dir_info
